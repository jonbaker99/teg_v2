...
- [ ] **Latest Round Scoreboard page-level overflow at 390px (source: P0-handoff.md, 2026-09-20).** P0's baseline measurement found `/latest-round` Scoreboard at 390px reports document width 402px in both light and dark, with `.lr-total-toggle` ending at x=402 — a 12px page-level overflow. P0 deliberately kept this out of F1-F5 scope and assigned it to "F3/F6 triage"; F3 (Scoring/Streaks) did not touch Scoreboard, and F6's review gate confirmed the finding as a real, still-open blocker rather than fixing it. Proposed owner: **C4** (responsive/table contracts), since it's a table/control-width contract issue rather than a one-off visual defect.
