...
- [ ] **C3 foundation — unresolved items, flagged for C4/C5 or a dedicated pass (2026-09-20).** From `webapp/design_reviews/ui_workstream/C3-handoff.md`, merged onto `main`:
  - **Global focus ring still green** — `ui-polish.css`'s sitewide `:where(a,button,select,input):focus-visible` rule resolves `--accent` (green); only the three new C3 control patterns (`.tab-underline`, `.seg-option`, `.action`) use `--focus` (`--ink`) instead. Out of scope for both C3 and C4 (owner decision, 2026-09-20) — needs a dedicated pass.
  ...
  - **Bare `.teg-table tbody tr.top-rank` still uses the old flat top-rank colour** on Records, History and other tables — the leader-tint fix (L2) is scoped to `.leaderboard-table`/`.results-table` only, by design. A site-wide retint (if wanted) is a `--table-toprank-bg` value change, not something C3 pre-wired.
  - **`webapp/theme.py`'s `TITLE_STYLES` list is now partly dead** — C3 deleted the CSS for title-style variants b/c/d/e/f1–f5 (superseded design-lab experiments); the design-lab selector still lists them but they now silently render as Style A. Minor cleanup, not urgent.
  - **Content-card padding/radius — comparison delivered, decision still open (C4, 2026-09-20).** `.main-content` still keeps its literal 40px padding / 4px radius; the tokenised alternative (`--sp-6` 32px / `--r-1` 3px) was **not applied**, only screenshotted for comparison: `webapp/design_reviews/ui_workstream/screenshots/C4/main-content-padding/`. Finding: visible but modest ~8px-per-side tightening in Clean Page, **zero effect in Clean Layered** (its `.page-panel .main-content` override already ignores this rule's padding/radius). Owner decides.
  - **History's disclosure button has no visible desktop affordance** — carried forward from C1b, still unresolved. C4 (2026-09-20) added `.action` to `button.teg-cell` for its colour/focus-ring tokens only (the row label keeps its own `color: inherit` so it doesn't turn green) — this does **not** decide the affordance question; still needs an owner call before any visible `+`/`-` desktop indicator is added.
