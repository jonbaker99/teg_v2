...
- [ ] **C3 foundation — unresolved items, flagged for C4/C5 or a dedicated pass (2026-09-20).** From `webapp/design_reviews/ui_workstream/C3-handoff.md`, merged onto `main`:
  ...
  - ~~**Bare `.teg-table tbody tr.top-rank` still uses the old flat top-rank colour** on Records, History and other tables — the leader-tint fix (L2) is scoped to `.leaderboard-table`/`.results-table` only, by design. A site-wide retint (if wanted) is a `--table-toprank-bg` value change, not something C3 pre-wired.~~ **Done (2026-09-21).** `base-vars.css`'s bare `.teg-table tbody tr.top-rank` now binds to `--leader-tint` (the same token `.leaderboard-table` uses), not `--table-toprank-bg` — that token was found to equal `--table-hover-bg` in both themes, which was the actual bug (a top-rank row was indistinguishable from a hovered one). Also updated the matching mobile sticky-first-column override in `mobile.css` to the same token so the pinned cell doesn't seam against the row.
