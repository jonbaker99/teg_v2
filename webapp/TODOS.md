...
- [ ] **C3 foundation — unresolved items, flagged for C4/C5 or a dedicated pass (2026-09-20).** From `webapp/design_reviews/ui_workstream/C3-handoff.md`, merged onto `main`:
  ...
  - **Content-card padding/radius decision still open** — `.main-content` keeps its literal 40px padding / 4px radius rather than the new `--sp-6` (32px) / `--r-1` (3px) tokens; C3 deliberately did not apply an unflagged pixel shift. C4 will produce a before/after screenshot comparison (owner decision, 2026-09-20) so the owner can decide without committing to the change first.
  - **History's disclosure button has no visible desktop affordance** — carried forward from C1b; the `+`/`−` indicator only shows ≤640px. Needs an owner call before C4 applies `.action` to it.
- [ ] **`.pill`/`.section-controls select` phone defaults below the 44px touch floor (source: C3-handoff.md, assigned to C4, 2026-09-20).** Both default to 40px in `mobile.css` (`:358`, `:366`) and only reach 44px today via page-scoped overrides. `webapp/design_principles.md`'s corrected hit-area contract (44px touch / 28–36px fine-pointer) makes this the two defaults' remaining gap.
- [X] **Latest Round Scoreboard page-level overflow at 390px (source: P0-handoff.md, fixed 2026-09-20).** The mobile Scoreboard switches now subtract both 12px gutters from their width, preserving 44px touch height and all table columns. Verified no page overflow at 320/375/390/430px in both Clean layouts and light/dark, including HTMX rank expansion, tab return, score-basis and chart-scale changes. Tablet/desktop at 768/1280px remain pixel-identical. [Handoff](design_reviews/ui_workstream/scoreboard-overflow-handoff.md). C4 may revisit the selector during systematic responsive/table work.
- [ ] **Scoring Heatmap page-level overflow at 390px (source: F5-followups-handoff.md, 2026-09-20).** The navigation cross-route sweep found `/scoring/heatmap` exceeds the viewport by 29px in Clean Page dark and 49px in Clean Layered light/dark; Clean Page light did not overflow in the sampled default state. This WIP page needs a focused selector/width diagnosis in a later responsive-table pass; it is unrelated to the sticky-nav delta guard and was not changed there.
