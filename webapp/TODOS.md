...
- [X] **Global focus ring still green (source: C3-handoff.md, fixed 2026-09-21).** `ui-polish.css`'s sitewide `:where(a,button,select,input):focus-visible` rule now resolves `var(--focus, var(--accent))` instead of `var(--accent)` — matching the `.tab-underline`/`.seg-option`/`.action` pattern, so a keyboard focus ring no longer reads as "selected" on a green-selected element. `--accent` kept as the fallback for any older theme without `--focus` defined. Verified via computed-style checks (not just visual) on `/leaderboard` and `/history`, Clean Page and Clean Layered, light and dark: plain links/buttons/selects now show `--focus` (`--ink`); the three existing exceptions are unchanged (they already won the cascade via class-selector specificity over the sitewide `:where()` rule, so this was never at risk of double-applying).
- [ ] **C3 foundation — unresolved items, flagged for C4/C5 or a dedicated pass (2026-09-20).** From `webapp/design_reviews/ui_workstream/C3-handoff.md`, merged onto `main`:
  - **`/results`' Net/Gross toggle still `.scale-switch`** — `/leaderboard`'s equivalent control was converted to the new `.segmented` pattern; `partials/results_table.html` was left on the old binary switch. C5's job per the roadmap; the conversion pattern is already proven on `/leaderboard`.
  - **Done (2026-09-21).** `base-vars.css`'s bare `.teg-table tbody tr.top-rank` now binds to `--leader-tint` (the same token `.leaderboard-table` uses), not `--table-toprank-bg` — that token was found to equal `--table-hover-bg` in both themes, which was the actual bug (a top-rank row was indistinguishable from a hovered one). Also updated the matching mobile sticky-first-column override in `mobile.css` to the same token so the pinned cell doesn't seam against the row. Note: no current route on Records/History actually applies `.top-rank` to a bare `.teg-table` yet, so this is future-proofing/consistency with the leaderboard rather than a fix to a visible page today.
  - [x] **`webapp/theme.py`'s `TITLE_STYLES` list is now partly dead** — C3 deleted the CSS for title-style variants b/c/d/e/f1–f5 (superseded design-lab experiments); the design-lab selector still lists them but they now silently render as Style A. **Removed dead entries 2026-09-21**; kept only a, e2 (live desktop styles) + m1, m2, m3 (mobile-only, wired to cookie).
  - [X] **History's disclosure button has no visible desktop affordance** — carried forward from C1b, closed 2026-09-21. Owner approved a visible `+`/`−` glyph matching `/latest-round`'s `.rank-toggle` pattern. Added a desktop-scoped (`@media (min-width: 641px)`) `::after` glyph directly on `button.teg-cell` in `base-vars.css`, reusing `.rank-toggle`'s exact tokens (`var(--accent, #17683c)`, 16px glyph, 28x28px hit area per `design_principles.md` → Hit areas) rather than inventing a new treatment; the button gains `display: flex` at that width to place the glyph after the row label. Scoped to desktop only so it doesn't duplicate mobile's existing column-based `.history-toggle-indicator` (`mobile.css`, unchanged). Verified live (Playwright): glyph renders correctly in both Clean Page/Clean Layered, light/dark; clicking flips `aria-expanded` to `true` and the `::after` content to "–"; at 390px the button's `::after` computes to `none` (mobile's own indicator still works, unaffected). `base-vars.css`'s cache-buster bumped `?v=34` → `?v=35` in `base.html`. `python -m pytest tests/test_webapp_pages.py -k history` — 3 passed.
- [X] **Content-card padding/radius tokenised (C4 follow-up, fixed 2026-09-20).** `.main-content` in `webapp/static/themes/clean-page.css` now reads `border-radius: var(--r-1)` (was literal `0.25rem`/4px) and `padding-left/right: var(--sp-6)` (was literal `2.5rem`/40px) — `--r-1` (3px) and `--sp-6` (32px) are defined in `clean.css`. Owner-approved after the C4 before/after comparison (`webapp/design_reviews/ui_workstream/screenshots/C4/main-content-padding/`): visible ~8px-per-side tightening in Clean Page, confirmed genuinely zero effect in Clean Layered (its higher-specificity `.page-panel .main-content` override still wins on both properties). Re-verified: `python scripts/check_css_comments.py` OK; `python -m pytest tests/test_webapp_pages.py -q` 103 passed; `/leaderboard` and `/records` render 200 with the tokenised CSS served as expected.
- [X] **`.pill`/`.section-controls select` phone defaults below the 44px touch floor (source: C3-handoff.md, fixed 2026-09-20 C4).** Both raised 40px→44px in `mobile.css`. Removed the now-redundant page-scoped 44px compensations this made obsolete on Latest Round/Latest TEG/Leaderboard/Results (`mobile.css`) and Player Profile's chart-control pills (`player-profile.css`).
- [X] **Latest Round Scoreboard page-level overflow at 390px (source: P0-handoff.md, fixed 2026-09-20; re-confirmed intact 2026-09-20 C4).** The mobile Scoreboard switches now subtract both 12px gutters from their width, preserving 44px touch height and all table columns. Verified no page overflow at 320/375/390/430px in both Clean layouts and light/dark, including HTMX rank expansion, tab return, score-basis and chart-scale changes. Tablet/desktop at 768/1280px remain pixel-identical. [Handoff](design_reviews/ui_workstream/scoreboard-overflow-handoff.md). C4's systematic responsive/table pass re-verified this formula is unchanged and found a *separate* small overflow in the toggle column instead (see the `.rank-toggle`/`.action` entry below).
- [X] **Scoring Heatmap page-level overflow at 390px (source: F5-followups-handoff.md, fixed 2026-09-20 C4).** Root cause was not the transposed table but `.hm-legend`'s colour-range label (`.hm-legend-range`, `white-space: nowrap` in an unwrapped flex row) — bled off-screen at ≤390px since nothing in its ancestry clipped horizontally. Fixed in `webapp/static/heatmap.css` (not `mobile.css`, which loads *before* this page-specific file and would have silently lost the cascade). Verified 0 overflow at 320/390/430px, Clean Page/Clean Layered, light/dark.
- [X] **`/scorecard` page-level overflow at 768px in Clean Layered (source: TODOS.md multi-round-scorecard entry, fixed 2026-09-20 C4).** `.sc-landscape:not(.data-card)` (`width: fit-content; margin: 0 auto`) sized itself to its un-clipped table content rather than the available column width, producing a 12px page overflow specific to the whole-TEG (Gross+Stableford stacked) landscape view. Fixed with `max-width: 100%` alongside the existing `width: fit-content` in `webapp/static/scorecard.css`. Verified: overflow gone at 768px; the title/table joint-centering this rule exists for is unaffected at 1280px; the wide "1 Player / All Rounds" view still scrolls internally rather than clipping.
- [X] **`.rank-toggle`/`.action` regression, found and fixed within the same C4 session (2026-09-20).** Adding `.action` to `.rank-toggle` (for its shared focus-ring/colour tokens) introduced a phone `min-width: 44px` floor via CSS's per-property cascade — the page-scoped `.rank-toggle` rule has higher overall specificity but never declared `min-width`, so `.action`'s value applied anyway and pushed the button past its narrow toggle-column, a 2–13px page overflow on `/latest-round`/`/latest-teg` at phone widths in Clean Page only. Fixed with an explicit `min-width: 0` in the page-scoped rule (`mobile.css`) — the button's touch target already comes from filling its column at 100% width/full row height, not from a literal min-width. Re-verified across the full 200-state matrix, 0 failures.
...
- [X] **Contents (`/`) page redesign — implemented as the current-TEG home, then revised twice
  after owner review (I3/I4/I5, 2026-09-21, revised 2026-09-22 x2).**
  ...
  genuine duplicate rows, unit-aware "Points"/"vs Par" header) + a compact
  gross-competition line, two-column (≥900px) next to a round-report teaser
  when one exists. **Complete:** when a tournament report exists, its own
  headline becomes the page's h1 (linked to the report) with a
  `TEG N results | Area | Month Year` dateline, a compact honours line
  (Champion/Green Jacket/Wooden spoon on one line above a hairline rule, not
  stacked rows), then the same two-panel treatment — real final standings
  (left) next to up to 4 of the report's other headlines under "Also in this
  report" (right); falls back to the original "TEG N — Final Results"
  treatment when no report exists. Both states' rich content shares one
  `_standings_table_context()` helper and always defers to
  `GET /contents/panel` via HTMX (only the complete-state headline, whose
  text depends on report availability, resolves synchronously) — reusing I1's
  `_standings_rows()`/`_standings_table.html` rather than a second renderer.
  The five `NAV_SECTIONS` groups (all 28 destinations, `/player` still
  unlinked) now sit inside a collapsible `<details>` disclosure below the
  state panel, closed by default with a signpost naming the real page count
  and group labels — reopening I3/I4's original "always visible" ruling at
  the owner's explicit request. Reports' `get_edition_summary()` gained
  `other_articles` (kicker+headline for every non-lead story). Went through
  three rounds of an approved interactive prototype
  (`https://claude.ai/artifact/UD49p88BHCfBTTNH4GRstz`) compared against
  Codex's independent `codex/contents-home-proposals` plan. Tests:
  `tests/test_webapp_pages.py` (14, `-k contents`). Full contract, rulings
  and rationale: `webapp/design_reviews/ui_workstream/I3-handoff.md`,
...
- [x] **Shared interaction states (I2, 2026-09-20)** — public read-only HTMX pages now commit tab/filter selection and canonical URL history only after successful swaps. One shared busy/error/retry contract preserves the prior view on failure. Direct links, reload, sharing, Back and forward retain declared public state. Latest Round keeps its separately audited pending/confirmed state machine and adopts only the shared feedback styling. [Handoff](design_reviews/ui_workstream/I2-handoff.md).
- [x] **Remove intrusive public loading treatment (2026-09-21).** Routine HTMX reads retain the confirmed view without inserting the `Loading view…` banner or fading the target. `aria-busy` remains for assistive technology; real failures still expose the stable Retry/Dismiss banner, and Retry is disabled while its exact GET is in flight.
...
- [ ] **`C4-handoff.md`'s audit table incorrectly records `/leaderboard`/`/results` as "Tier 2 card reflow (`lb_cards.html`), desktop table hidden | Compliant"** (found during I1). That was never true post-R3.2 — the desktop table was always what rendered at every width; the card markup was dead. Historical handoffs are left as-is (not edited retroactively); noting the discrepancy here so a future audit doesn't repeat it.
...
- [X] **Latest TEG metric row uneven/too tall on phones; Latest Round and Latest TEG used different controls for the same job** (2026-09-22, Jon). Root cause was a dead CSS rule: the Latest TEG template had migrated to `.segmented`/`.seg-option` but `mobile.css` still targeted the old `.metric-grid`/`.metric-pill` classes, so nothing styled the row and four long labels wrapped inside a content-sized `inline-flex` bar. Both pages now share `.segmented` plus a new `.segmented--grid` modifier (even two-column grid below 640px, inert above); `.metric-grid`/`.metric-pill` is retired as a control (the unrelated headline-metric-card `.metric-grid` in `base-vars.css` is untouched). Latest Round's Scoring pills moved to the same segmented grammar at the same time.
- [X] **Records & PBs showed `G.Par` / `N.Par` on phones** (2026-09-22, Jon). `_build_records_html` ran `_player_name_spans` over the identity column unconditionally, but in the Personal Bests/Worsts sections that column holds a metric friendly name, so "Gross vs Par" was abbreviated as if it were a person. Fixed with an `identity_is_player` flag defaulting to the old behaviour, leaving `/records` unchanged. First column gained a left margin and the final column is now left-aligned (both shared with `/records`, intentionally).
- [X] **Scoring table first column header leaked the internal field id** (2026-09-22). `_format_scoring_display`'s `reset_index()` put the pandas index name (`GrossVP`/`Stableford`) straight into the header. Now reads `Score` in both views on both pages, at every width; also retires the `white-space: normal` wrap hack that existed only to cope with that long header.
- [X] **`/leaderboard` and `/results` Scorecards: one toggle per round, every round expanded** (2026-09-22, Jon). Now one page-level Gross/Stableford selector plus a collapsible container per round (Round 1 open), both phone-only. Rounds render `open` server-side so desktop and the no-JS path are byte-equivalent to before. Note the page-level wrapper must NOT carry `.sc-portrait` (it holds every round block, and `.sc-portrait` is `display:none` above 640px — that would blank the tab on desktop), and its pane rules select through the round wrapper with `~ *`, so each round must stay a direct child of it.
...
- [X] **`.rank-toggle` glyph missing above 640px** (2026-09-19, fixed 2026-09-20 F1; corrected 2026-09-20 C1). Added a desktop-scoped counterpart in `base-vars.css` (glyph `::after` content + a 28x28px hit area) alongside the existing `mobile.css` phone rule, following the same mobile/desktop split already used for `table.leaderboard`. F1's "390px phone sizing/behaviour unchanged" claim was verified for `min-height` only — the desktop rule was left unscoped, so its `min-width: 28px` floor leaked into phone widths too (nothing in `mobile.css`'s equal-specificity, later-loaded rule overrides `min-width`). C1 wrapped the desktop rule in `@media (min-width: 641px)`. Re-verified on `/latest-round` at 390px (`min-width: 0px`, `min-height: 44px`, glyph present) and 641px (28x28, glyph present, click still flips `aria-expanded`), both Clean layouts.
