...
- [ ] **`.rank-toggle` glyph missing above 640px** (2026-09-19). The `/latest-round` Scoreboard table's `+`/`-` expand button for the detail row (Out/In split, score mix) is styled only in `mobile.css`, including the `::after` pseudo-element that draws the glyph — on desktop widths (>640px) it renders as an empty, unlabelled button, making the whole expandable detail row effectively inaccessible/invisible above 640px. Needs a desktop-width style for the toggle glyph.
- [ ] **Report tab still inline in the tab bar on `/results` and `/latest-teg`** (2026-09-19). `/latest-round`'s Report link was just moved out of the tab bar to its own row below it, with distinct styling (dashed-underline italic serif) making clear it navigates away, rather than a `.tab-underline`-styled link that looks same-page. `/results` and `/latest-teg` still render Report inline in their tab bars — worth applying the same treatment for consistency.
