...
- [X] **Long-page public navigation persistence (F5, 2026-09-20).** Fixed: `webapp/templates/base.html`'s sticky-nav scroll handler computed hide offset from absolute `scrollY` past the 300px threshold, not from scroll direction, so on a long page it stayed hidden on the way back up until `scrollY` dropped back below `threshold + navHeight`. Now tracks the previous `scrollY` and shows the nav immediately (`top:0`) on any upward scroll; downward-past-threshold hiding is unchanged. Verified on `/scoring/all-rounds?n=100`, `/records`, `/latest-round` at 390/768/1280px, light/dark: nav reappears at scroll-up, no content jump (sticky positioning, no reflow), no overlap with content or the phone bottom tab bar, resize/HTMX-swap regressions checked. `python -m pytest tests/test_webapp_pages.py -k 'nav_page_renders' -v` — 27 passed. Screenshots: `webapp/design_reviews/ui_workstream/screenshots/F5/`.
