...
| `milestone_records.py` | `detect_streak_records(teg)` — TEG-level streaks (Eagles, Birdies, Pars-or-better, TBPs, +2s-or-worse, Over-par) that tie/break the all-time record. `detect_personal_streak_extremes(teg)` — a player's own longest-ever streak (best or worst direction) set this TEG, vs. their prior TEGs. `detect_score_count_records(teg)` — TEG-total scoring-category counts (most Eagles/Birdies-or-better/Pars-or-better, most TBPs) that tie/break the all-time record. Wraps `analysis/streaks.py` and `analysis/records.py` (the webapp Records page's own detection) | New `sr*`/`sc*` **mandatory beats** for the LLM bundle; also feeds `render.build_records_block`'s Notable Achievements appendix directly (2026-09-12) |
...
instead of seven.** Its sibling `--from draft` reuses the plan and redrafts the sections (six calls),
for when the drafting prompt is what you are changing. That is what `restyle_voice`'s `voice_prompt` argument and
...
6. **Choose which stories are printed** — optional, and applied last.
   ...
   **Row packing (2026-09-11)** replaced the old `choose_arrangement`/e1/e2/e3 fixed-arrangement
   system, which produced an orphan-column bug on some article counts. `newspaper_edition.plan_rows(subs)`
   packs the remaining sub-cards into `<div class="subs-row cols-N">` rows explicitly instead of
   picking from a small set of named layouts — one function, one renderer, no per-arrangement CSS.
...
- `<section class="callout at-a-glance-box">` with Trophy/Jacket/Spoon winners (computed from scores, not the plan — see below)
...
- **Standings.** `build_round_standings(teg)` computes end-of-round standings; they are injected under each `## Round N` heading. If the writer took a theme-led route with no round headings, a consolidated "Standings by round" appendix is inserted before the player closing instead. Each entry is the **cumulative total with that round's own score in brackets** — `SN 156 (R4: 43)` — so the block answers both "who is winning" and "who had a good day". Round 1 has no bracket, because there the cumulative total *is* the round score. The newspaper edition's compact rail strips the brackets (`_totals_only`); the appendix table keeps them. The **final round's order is override-aware** (`analysis.history.get_teg_placings`, see below) — it agrees with `/history` on a tiebreak decided off-course (e.g. TEG 5's Green Jacket).
- **At-a-glance.** `_build_at_a_glance(teg)` says the same three things every time: winner, their ordinal win count (`history_context.build_win_counts()`, e.g. "3rd Trophy"), nothing else — no score, no margin. The winner comes from `analysis.history.get_teg_placings(all_data, teg)`, which sorts on the era Trophy metric and Gross, and is override-aware the same way `analysis.history.get_teg_winners` (used by `/history`) is — **not** from the LLM plan's `competitions[]`, which used to carry ad hoc free text and drifted inconsistent across the corpus. Runner-up is added downstream by `newspaper_edition._add_runners_up`, reading the Standings appendix.
- **Notable Achievements appendix.** `build_records_block(teg, round=None)` appends a `class="records"` inventory: TEG records, personal bests, personal worsts and rare feats — best **and** worst, TEG-record **and** personal, throughout. Sources, all shared with the webapp Records page rather than reimplemented:
  - Round-level records/PBs/worsts and single-hole feats (aces, eagles, blow-ups) — `events.build_notable_events` (`events._round_beats`'s own `has_history` gate, plus an all-time-rank pool-size gate added 2026-09-12).
  - Tournament totals (Trophy + Gross, era-aware; never raw Score) — `analysis.records.identify_aggregate_records_and_pbs`.
  - Nine-hole records/PBs/worsts — `analysis.records.identify_9hole_records_and_pbs`.
  - Streak records (all-time ties/breaks) and personal-best/worst streaks — `reporting.milestone_records.detect_streak_records` / `detect_personal_streak_extremes`, wrapping `analysis.streaks`.

  History-depth gated (`render.py`'s `MIN_PRIOR_TEGS_FOR_LEAGUE_RANK` and siblings) so an early TEG doesn't drown in trivial claims — a debut round or total is not a meaningful "personal best", and "3rd-best in TEG history" means nothing off a 2-TEG pool. No cap on volume otherwise. Hole scores over par are numeric ("+5"), never a word ladder ("quintuple bogey") — under-par terms (eagle, albatross, hole-in-one) are kept, since those are names for the thing, not inflation. De-duplicated across rounds (`_dedup_entries`, round suffixes sorted ascending).
...
call, `CORRECTIONS_CONTRACT` + the rule constants, permitting exactly three edits: delete a rank
claim the ranking rule disallows, expand an ambiguous name or first competition mention, or add
one sentence stating the double (only where the supplied `double` data shows one and the lead
story doesn't already say so — the one exception to "add nothing"). `apply_corrections` computes
`history_context.build_double_context(teg_num)` itself and hands the real figures to the model in
the user message, so a rarity claim is never invented.
