...
- [ ] **Break storyline-first's dependency on the legacy story plan** — `render.style_text()` calls `authoring.load_story_plan()`, which opens `teg_N_story_plan.json`, so styling a storyline-first report needs a *legacy* plan for the same TEG. Invisible today because TEGs 14/16/18 have both; `FileNotFoundError` on any TEG that doesn't. **Blocks generating storyline-first reports for the other 14 TEGs**, and so blocks the `/teg-reports` switch-over. `style_text` uses the plan for exactly one thing — `_build_at_a_glance` reads `plan["competitions"]` — and `StorylinePlan` already carries `competitions[]` in the identical shape (`{name, winner_or_loser}`), so the fix is to load the storyline plan when styling a storyline-first report rather than to change any schema.
- [ ] **Teach `promote_variant` the storyline-first filenames** — `paths._artefact_names` lists only the legacy five, so a storyline-first variant is never promoted: silently skipped when legacy files are present, a bare `FileNotFoundError` when they are not. Small, self-contained.
