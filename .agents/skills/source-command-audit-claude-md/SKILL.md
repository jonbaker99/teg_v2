---
name: "source-command-audit-claude-md"
description: "Audit AGENTS.md for drift — contradictions with the codebase, dead references, and instructions made obsolete by newer capabilities"
---

# source-command-audit-claude-md

Use this skill when the user asks to run the migrated source command `audit-claude-md`.

## Command Template

Audit `AGENTS.md` for drift. Do not edit anything yet — report first, then ask what to fix.
...
For every factual claim in `AGENTS.md`, check it against the repo:
...
This is the pass that's easy to skip and matters most. Check current Codex documentation (web search if needed) and identify:
...
Then ask which fixes to apply. When applying them, update the freshness date at the top of `AGENTS.md` to today.
