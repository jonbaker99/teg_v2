...
- **Design round 2 exploration** (`design_round2/`): standalone prototypes for the full visual reimagine — 3 themes (Plain / Editorial / Striking) × 3 stylings each, rendered on real Results + Player pages with per-theme component sheets, light+dark per styling, token-driven Plotly re-theming, and screenshots. Self-contained (vendored Plotly/Tailwind), not wired into the webapp. See `design_round2/README.md` for the styling matrix and the proposed new tokens (`--font-data`, `--table-header-text`, `--table-tier*-*`, `--chart-*`).
