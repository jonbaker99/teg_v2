...
- **Mono theme (2026-07-13, branch `claude/design-theme-serif-fonts-0tqb5o`):** a new default site design — a monospace "record-book" look (plain white/near-black page, no cards, shaded data **bands**, one forest-green accent, mono data + sans UI) built to the handoff spec in `webapp/design_handoff`. Implemented purely token-driven in `webapp/static/themes/mono.css` (declares the handoff palette, maps it onto the existing base-vars token names via `var()`, adds the band/table/cell look rules) — **no edits to `base-vars.css` or any template's markup**. Per request, the **site title, main nav and page (H1) titles retain the site's Lora serif**; everything else follows the Mono sans/mono split. Mono owns its own neutral-grey dark palette under `html[data-mode="dark"][data-theme="mono"]`; `dark.css` was scoped to `:not([data-theme="mono"])` (behaviour-identical for the clean themes) and `base.html` now tags `<html>` with `data-theme`. Registered in `theme.py` (`DEFAULT_THEME = "mono"`, Plotly frame colours added). **Fully reversible:** the previous design is untouched as the `clean-page` theme — set `DEFAULT_THEME = "clean-page"` to revert; pristine backups + revert guide in `webapp/static/themes/archive/pre-mono-backup/`. Verified light + dark on Results and Player pages; 360 tests pass. **Follow-up (cookie migration):** a persisted `theme=clean-page` cookie was overriding the new default on returning browsers (same commit, different UI on two machines). Added theme-cookie versioning in `theme.py` — `get_theme` honours a `theme` cookie only when a `theme_ver` cookie matches `THEME_COOKIE_VERSION` (now `"2"`), else migrates stale/legacy cookies to `DEFAULT_THEME`; `set_theme_cookies(response, theme)` writes both together for a future switcher. Bump `THEME_COOKIE_VERSION` whenever a `DEFAULT_THEME` change should re-migrate existing selections.
