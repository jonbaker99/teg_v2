...
- **Webapp**: Full Streamlit page set replicated. Data-admin is now ported behind cookie auth — **add a round**, **edit metadata CSVs** and **delete rounds** (`webapp/routes/admin.py`, all driven by the headless `teg_analysis/analysis/data_update.py`); report generation and volume management remain out of scope. Nav mirrors Streamlit's sections/titles/ordering via a single source of truth, `webapp/nav.py` (`NAV_SECTIONS`), injected into templates and looped in `base.html`; `/` lands on the new **Contents** site map. Newly added: **Contents** (`/contents`) and **Eclectic Records** (`/eclectic-records`, backed by new helpers in `teg_analysis/analysis/eclectic.py`). All functional. TEG Reports page at `/teg-reports` and the Report tab on `/results` render the new commentary reports.
