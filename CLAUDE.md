...
- **Commentary / report pipeline** (`teg_analysis/reporting/`): LLM-powered tournament + round reports built around a 5-stage pipeline — scored evidence-carrying beats, competition arcs, venue/era/career/course context (code) → structured story plan with named narrative vehicles and setup→payoff pairs (LLM) → dry draft as QA scaffold + entertaining write-up + repetition lint (LLM) → CSS-class styled markdown with deterministic standings and records blocks injected. Cost ~$0.65/report on Opus 4.7. **Tournament coverage is complete — TEGs 2–18 all published** (there is no TEG 1 in the data) and rendering at `/teg-reports`; round reports exist for TEGs 8/9/10 (all rounds), 11 (R1–R2), 14 (R1/R4), 18 (R3) — 17 of 67. The library spans several pipeline vintages (TEGs 2–8, 15, 16 and all round reports predate the narrative-vehicle work), and **work stopped mid-experiment on a humour-dial A/B whose verdict was never recorded** — that decision blocks regeneration. **How it works**: `teg_analysis/reporting/README.md`. **Where we got to / what's next**: `teg_analysis/reporting/STATUS.md` (the pick-up ledger). **Experiment log**: `teg_analysis/reporting/EXPERIMENTS.md`.
