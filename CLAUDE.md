...
2. Implement the simplest solution for simple problems, better solutions for harder problems. Do not over-engineer or add flexibility that isn't needed yet. Use existing patterns and components from the codebase over inventing new ones. Prefer minimal, focused changes over comprehensive rewrites, unless a rewrite would significantly simplify the codebase — ask "what's the smallest change that solves this?" first.
...
**Done:** `teg_analysis/` is the canonical, UI-agnostic analysis layer (no Streamlit imports). `webapp/` (FastAPI + HTMX + Jinja2 + Tailwind) is the deployed site on Railway and has full functional parity with the Streamlit app, including: data-admin (add/delete rounds & TEGs, metadata edits, GitHub↔volume sync, backups browser, file guide — `webapp/routes/admin.py`); a guided round-setup wizard (`webapp/README.md` § "New round"); native mobile live-round score entry with conflict handling and a live leaderboard (`teg_analysis/analysis/live_round.py`, `webapp/README.md` § "Live round entry"); an LLM tournament-report pipeline (`teg_analysis/reporting/README.md` + `STATUS.md`); and a reworked player-profile section (functionally complete, UI polish pending — `webapp/TODOS.md`). Remaining Streamlit-parity gaps are cosmetic only, tracked in `webapp/PARITY_AUDIT.md`.

**Next priorities:**
1. **Mobile UI + dark mode** — native-app feel (bottom tab bar, sticky app bar, reflowed data), without changing laptop/iPad render. Plan + progress: `webapp/MOBILE_PLAN.md`; scorecard work package: `webapp/SCORECARD_PORT.md`.
2. **Close remaining cosmetic parity gaps** — `webapp/PARITY_AUDIT.md`.
3. **Webapp formatting/visual-polish pass** — number formatting, table styling, layout consistency.
4. **REST API** — `/api` layer over `teg_analysis` so non-Python clients can use it; currently a placeholder.
5. **Retire Streamlit** — once the REST API + webapp are production-ready.

For what changed in a given session, use `git log` rather than this file — this section tracks current state only, not history.
...
Guidance for picking a model on substantial work — not a gate on every message. Skip entirely for quick questions, one-line fixes, or when running unattended/in Auto mode (General Rule 1 already covers proceeding on reasonable judgement there).
...
### When to flag a mismatch

Before starting a genuinely **complex** task on a non-Opus model, say so in one line and continue unless told otherwise:

```
**Model note:** this looks like complex/architectural work → consider `/model opus`. Continuing on [current model] unless you'd rather switch.
```

For simple/moderate work, or when a task shifts tier mid-stream, a brief note is enough — no need to stop and wait.

### Opus review gate

After Haiku or Sonnet complete a batch of non-trivial work, consider an Opus pass to review it.
