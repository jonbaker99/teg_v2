...
# Calculate the total column (average across all players for each course)
summary['Total'] = summary.mean(axis=1)
...
columns = ['Course'] + [col for col in summary.columns if col != 'Course']
summary = summary[columns]

# Function to format numbers with 1 decimal place
def format_number(x):
    if pd.isna(x):
        return '-'
