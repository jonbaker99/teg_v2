...
`course_pars.csv` was backfilled once from `all-scores` + `round_info` history (`scripts/backfill_course_pars.py`) and is now edited like any other metadata CSV. Two courses (Boavista, Praia D'El Rey) had one historical round each that disagreed with every other round on all 18 holes — resolved by majority vote, with the disagreeing round flagged for a separate look at the underlying `all-scores.parquet` records (not changed by the backfill). Estoril has a genuine tie (only 2 rounds ever played there, each disagrees with the other) and is deliberately incomplete pending a manual decision — see `DATA_STORAGE_INGESTION_PLAN.md`.
